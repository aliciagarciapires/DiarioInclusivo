...
if (
    !file_exists($fileException) ||
    !file_exists($filePHPMailer) ||
    !file_exists($fileSMTP)
) {
...
$emailInst = $dados['email_institucional'] ??
             $dados['emailInstitucional'] ??
             '';

$emailAdm = $dados['email_adm'] ??
            $dados['emailAdm'] ??
            '';
...
$nomeEscola = filter_var(
    $nomeEscola,
    FILTER_SANITIZE_SPECIAL_CHARS
);

$emailInst = filter_var(
    $emailInst,
...
if (!$emailInst || !$emailAdm) {
    ...
    echo json_encode([
        "sucesso" => false,
        "mensagem" => "Insira e-mails validos."
    ]);
...
try {
    ...
    // Resposta vai para o e-mail da instituição
    $mail->addReplyTo(
        $emailInst,
        $nomeEscola
    );
    ...
    $mail->Subject =
        'Nova Solicitacao de Cadastro - Diario Inclusivo';

    $bodyHtml =
        "<h2>Nova Solicitacao de Cadastro de Administrador</h2>";

    $bodyHtml .=
        "<p><b>Nome da Escola:</b> " .
        htmlspecialchars($nomeEscola) .
        "</p>";

    $bodyHtml .=
        "<p><b>E-mail Institucional:</b> " .
        htmlspecialchars($emailInst) .
        "</p>";

    $bodyHtml .=
        "<p><b>E-mail do Administrador:</b> " .
