...
export default function CadDiscente() {
  ...
  const validarDataNascimento = (dataString: string) => {
    ...
    if (partes.length !== 3) return { valida: false, mensagem: 'Formato de data invÃ¡lido.' };

    const dia = parseInt(partes[0], 10);
    const mes = parseInt(partes[1], 10);
    const ano = parseInt(partes[2], 10);

    if (mes < 1 || mes > 12) return { valida: false, mensagem: 'MÃªs invÃ¡lido.' };
    ...
    if (dataObjeto > hoje) return { valida: false, mensagem: 'Data nÃ£o pode ser no futuro.' };
    if (ano < 1900) return { valida: false, mensagem: 'Ano invÃ¡lido.' };
  ...
  useEffect(() => {
    const buscarResponsaveis = async () => {
      try {
        ...
        const response = await fetch('http://192.168.0.107/DiarioInclusivo/src/app/buscar_responsaveis.php');
        ...
        console.error("Erro na busca de responsÃ¡veis:", error);
        setListaResponsaveis([]);
      }
    };
 ...
 const cadastrarDiscente = async () => {
  ...
  try {
    const response = await fetch('http://192.168.0.107/DiarioInclusivo/src/app/cadDiscente.php', {
    ...
    if (resultado.success) {
      ...
      Alert.alert('Erro', resultado.message || 'NÃ£o foi possÃ­vel realizar o cadastro.');
    }
  } catch (error) {
    console.error(error);
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 110 }}>
        <View style={styles.container}>
          ...
          <View style={styles.form}>
            ...
            <Text style={styles.textoInput}>ResponsÃ¡veis:</Text>
              ...
              <Pressable style={styles.select} onPress={() => setEditando(true)}>
                <Text style={styles.selectTexto}>
                  {responsaveisSelecionados.length > 0 
                    ? `+ Adicionar/Alterar ResponsÃ¡veis (${responsaveisSelecionados.length})` 
                    : "Selecione os responsÃ¡veis"}
                </Text>
              </Pressable>
            )}
