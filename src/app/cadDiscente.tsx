...
export default function CadDiscente() {
  ...
  useEffect(() => {
    const buscarResponsaveis = async () => {
      try {
        const response = await fetch('http://10.0.0.102/DiarioInclusivo/src/app/buscar_responsaveis.php');
 ...
 const cadastrarDiscente = async () => {
  ...
  try {
    const response = await fetch('http://10.0.0.102/DiarioInclusivo/src/app/cadDiscente.php', {
      method: 'POST',
