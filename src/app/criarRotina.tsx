...
export default function CriarRotina() {
  ...
  // FUNÇÃO: Salva a atividade que o usuário digitou manualmente
  const salvarAtividadePersonalizada = async () => {
    if (novoNomeAtividade.trim() === '') return; // Impede salvar em branco

    try {
      const URL_API = 'http://192.168.1.59:8080/AulaemPHP/criar_atividade.php';
  ...
  const finalizarRotina = async () => {
    ...
    try {
      ...
      const URL_SALVAR = 'http://192.168.1.59:8080/AulaemPHP/salvar_rotina.php';
