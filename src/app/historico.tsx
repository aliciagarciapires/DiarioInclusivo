...
export default function HistoricoScreen() {
  ...
  const buscarHistorico = async () => {
    try {
      ...
      setCarregando(true);
      const url = `http://192.168.0.103/DiarioInclusivo/src/app/listar_diario.php?idDiscente=${idDiscenteSelecionado}`;
  ...
  const buscarAtividadesDoBanco = async () => {
    try {
      const url = `http://192.168.0.103/DiarioInclusivo/src/app/listar_atividades.php`;
      const resposta = await fetch(url);
      const textoPuro = await resposta.text();
      const json = JSON.parse(textoPuro);
  ...
  const excluirHistorico = async (idDiario: number) => {
    try {
      const response = await fetch(`http://192.168.0.103/DiarioInclusivo/src/app/deleteHistorico.php`, {
        method: "POST",
  ...
  const salvarEdicao = async (idDiario: any) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.0.103/DiarioInclusivo/src/app/update_diario.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
