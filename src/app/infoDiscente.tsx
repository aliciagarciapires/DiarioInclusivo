...
export default function InfoDiscente() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const resResp = await fetch("http://200.18.141.168/DiarioInclusivo/src/app/buscar_responsaveis.php");
      ...
      const response = await fetch(`http://200.18.141.168/DiarioInclusivo/src/app/getDiscente.php?id=${id}`);
  ...
  const salvarAlteracoes = async () => {
    ...
    try {
      ...
      const response = await fetch("http://200.18.141.168/DiarioInclusivo/src/app/updateDiscente.php", {
        method: "POST",
  ...
  const deletarDiscente = async () => {
    try {
      ...
      const response = await fetch(
        `http://200.18.141.168/DiarioInclusivo/src/app/deleteDiscente.php?id=${id}&idUsuario=${idUsuarioLogado}`,
        { method: "DELETE" }
      );
      const json = await response.json();
