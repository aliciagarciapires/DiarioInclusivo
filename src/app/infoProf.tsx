...
export default function InfoProf() {
      ...
      try {
        ...
        if (idLogado) {
          const respLogado = await fetch(
            `http://200.18.141.168/DiarioInclusivo/src/app/getUsuario.php?id=${idLogado}`
          );
        ...
        const response = await fetch(
          `http://200.18.141.168/DiarioInclusivo/src/app/getUsuario.php?id=${idFinal}`
        );
  ...
  const salvarEdicao = async () => {
    ...
    try {
      setSalvando(true);
            const response = await fetch(
        "http://200.18.141.168/DiarioInclusivo/src/app/updateUsuario.php",
        {
          method: "POST",
  ...
  const apagarProfessor = async () => {
    try {
      ...
      setLoading(true);
            ...
            const response = await fetch(
        `http://200.18.141.168/DiarioInclusivo/src/app/deleteUsuario.php?id=${prof.idUsuario}`,
        { method: "GET" }
            );
      const json = await response.json();
