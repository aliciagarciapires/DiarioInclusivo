...
export default function InfoUsuario() {
    ...
    useCallback(() => {
      const buscarDadosContaLogada = async () => {
        try {
          ...
          const response = await fetch(`http://192.168.1.5/DiarioInclusivo/src/app/getUsuario.php?id=${idFinal}`);
  ...
  const salvarEdicao = async () => {
    ...
    try {
      setSalvando(true);
      const response = await fetch("http://192.168.1.5/DiarioInclusivo/src/app/updateUsuario.php", {
        method: "POST",
  ...
  const apagarUsuario = async () => {
    try {
      ...
      if (!idParaDeletar) {
        ...
        `http://192.168.1.5/DiarioInclusivo/src/app/deleteUsuario.php?id=${idParaDeletar}`,
        { method: "GET" }
            );
