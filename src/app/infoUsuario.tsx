...
export default function InfoUsuario() {
    ...
    useCallback(() => {
      const buscarDadosContaLogada = async () => {
        try {
          ...
          const response = await fetch(`http://10.0.0.100/DiarioInclusivo/src/app/getUsuario.php?id=${idFinal}`);
  ...
  const salvarEdicao = async () => {
    ...
    try {
      setSalvando(true);
      const response = await fetch("http://10.0.0.100/DiarioInclusivo/src/app/updateUsuario.php", {
        method: "POST",
  ...
  const apagarUsuario = async () => {
    try {
      ...
      const response = await fetch(
        `http://10.0.0.100/DiarioInclusivo/src/app/deleteUsuario.php?id=${idParaDeletar}`,
        { method: "GET" }
      );
