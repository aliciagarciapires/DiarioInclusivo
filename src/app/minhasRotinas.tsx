...
export default function VisualizarRotina() {
  ...
  const carregarRotinasDoBanco = async (idUser: string) => {
    try {
      setCarregando(true);
      const URL_API = `http://192.168.0.103/DiarioInclusivo/src/app/listar_rotina.php?idUsuario=${idUser}&t=${new Date().getTime()}`;
  ...
  const carregarAtividadesMaster = async () => {
    try {
      const URL_MASTER = `http://192.168.0.103/DiarioInclusivo/src/app/buscarAtividades.php?t=${new Date().getTime()}`;
          ...
          onPress: async () => {
            try {
              const resposta = await fetch(
                `http://192.168.0.103/DiarioInclusivo/src/app/deletar_rotina.php`,
                {
                  method: "POST",
  ...
  const salvarEdicaoRotina = async () => {
    ...
    try {
      ...
      const URL_UPDATE = `http://192.168.0.103/DiarioInclusivo/src/app/updateRotina.php`;
